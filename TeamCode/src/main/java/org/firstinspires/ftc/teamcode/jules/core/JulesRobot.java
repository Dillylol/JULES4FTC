...
public class JulesRobot {
    ...
    private void initHardware() {
        try {
            ...
            if (leftFront != null) leftFront.setDirection(JulesConstants.Motors.LEFT_DIR);
            if (leftRear != null) leftRear.setDirection(JulesConstants.Motors.LEFT_DIR);
            if (rightFront != null) rightFront.setDirection(JulesConstants.Motors.RIGHT_DIR);
            if (rightRear != null) rightRear.setDirection(JulesConstants.Motors.RIGHT_DIR);
    ...
    public void processCommands() {
        ...
        while ((line = subscription.poll()) != null) {
             try {
                 com.google.gson.JsonObject event = org.firstinspires.ftc.teamcode.jules.bridge.util.GsonCompat.parse(line).getAsJsonObject();
    ...
    public void executeCommand(String json) {
        ...
        try {
            com.google.gson.JsonObject cmd = org.firstinspires.ftc.teamcode.jules.bridge.util.GsonCompat.parse(json).getAsJsonObject();
            String type = cmd.has("type") ? cmd.get("type").getAsString() : "";
            
            // Handle "type": "motor"/"servo" (Low-Level)
            // String type = cmd.has("type") ? cmd.get("type").getAsString() : ""; // Already defined above
            ...
            if (cmd.has("name") && "drive".equals(cmd.get("name").getAsString())) {
                if (cmd.has("args")) {
                    ...
                    // NOTE: duration_ms handling requires async scheduling which isn't implemented yet.
                    // This command sets immediate power. The sender (LLM) should send a stop command (t=0) later,
