...
abstract class BaseTagShooterTele extends BjornTele {
    ...
    private static final double RPM_OFFSET = 2084.2966941424975;
    ...
    public void loop() {
        ...
        ShooterSolution solution = tracking ? computeShooterSolution(obs) : null;
    ...
    private ShooterSolution computeShooterSolution(TagObservation obs) {
        ...
        double rpm = clampManualRpm(RPM_SLOPE * rangeFeet + RPM_OFFSET);
        double yawGain = 1.0 + Math.abs(thetaDeg) * 0.002; // slight boost for angled shots
        rpm = clampManualRpm(rpm * yawGain);
        return new ShooterSolution(rpm, thetaDeg, rangeFeet, lateralFeet, obs.id);
    }
