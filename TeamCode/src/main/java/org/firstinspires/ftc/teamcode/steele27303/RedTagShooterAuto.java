...
@Autonomous(name = "BjornAutoRED", group = "Bjorn")
public class RedTagShooterAuto extends OpMode {
    ...
    public void init() {
        ...
        follower.setStartingPose(BjornConstants.AutoPoses.Red.START);
        ...
        switch (state) {
            ...
            case GRAB_ROW: // Actually SHOOTING
                if (runShootPhase(nowMs)) {
                    ...
                    // We are at Shoot Zone. Path to Row Start is valid.
                    PathChain shootToRowStart = line(BjornConstants.AutoPoses.Red.SHOOT_ZONE,
                            BjornConstants.AutoPoses.Red.ROW_START);
            ...
            case TO_ALIGN1_BACK:
                if (follower.atParametricEnd()) {
                    ...
                    PathChain alignToShoot = line(BjornConstants.AutoPoses.Red.ALIGN1_BACK,
                            BjornConstants.AutoPoses.Red.SHOOT_ZONE);
    ...
    private void buildStaticPaths() {
        toShoot = line(BjornConstants.AutoPoses.Red.START, BjornConstants.AutoPoses.Red.SHOOT_ZONE);
        toGlance = line(BjornConstants.AutoPoses.Red.SHOOT_ZONE, BjornConstants.AutoPoses.Red.GLANCE_POINT);
        toRowStart = line(BjornConstants.AutoPoses.Red.GLANCE_POINT, BjornConstants.AutoPoses.Red.ROW_START);

        // Fallback Paths
        toAlign1 = line(BjornConstants.AutoPoses.Red.GLANCE_POINT, BjornConstants.AutoPoses.Red.ALIGN1);
        toGrab1 = line(BjornConstants.AutoPoses.Red.ALIGN1, BjornConstants.AutoPoses.Red.GRAB1);
        toAlign1Back = line(BjornConstants.AutoPoses.Red.GRAB1, BjornConstants.AutoPoses.Red.ALIGN1_BACK);
        toPark = line(BjornConstants.AutoPoses.Red.SHOOT_ZONE, BjornConstants.AutoPoses.Red.PARK);
    }
    ...
    private PathChain buildRowPath(int row) {
        ...
        switch (row) {
            case 1:
                target = BjornConstants.AutoPoses.Red.ROW1;
                break;
            case 2:
                target = BjornConstants.AutoPoses.Red.ROW2;
                break;
            case 3:
                target = BjornConstants.AutoPoses.Red.ROW3;
                break;
            default:
                target = BjornConstants.AutoPoses.Red.ROW1;
                break;
        }
        return line(BjornConstants.AutoPoses.Red.ROW_START, target);
    }
    ...
    private PathChain buildReturnRowPath(int row) {
        ...
        return line(target, BjornConstants.AutoPoses.Red.ROW_START);
    }

    private PathChain buildShootRowPath() {
        return line(BjornConstants.AutoPoses.Red.ROW_START, BjornConstants.AutoPoses.Red.SHOOT_ZONE);
    }
    ...
    private boolean runShootPhase(long nowMs) {
        ...
        if (scanLeft > 0) {
            ...
            if (scanLeft == 0) {
                ...
                List<AprilTagCamera.TagObservation> tags = aprilTagCamera.pollDetections();
                for (AprilTagCamera.TagObservation tag : tags) {
                    if (tag.id == CameraConfig.RED_GOAL_TAG_ID) {
                        distCV = tag.z * 3.28084;
                        break;
                    }
                }
                double distPedro = Math.abs(BjornConstants.AutoPoses.Red.SHOOT_ZONE.getY() - follower.getPose().getY())
                        / 12.0;
